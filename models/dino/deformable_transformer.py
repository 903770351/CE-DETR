...
class DeformableTransformer(nn.Module):
    ...
    def inverse_transform(self, src_flatten, mask_flatten, lvl_pos_embed_flatten, spatial_shapes):
        srcs = []
        masks = []
        pos_embeds = []
        ...
        for spatial_shape in spatial_shapes:
            end_idx = start_idx + spatial_shape[0] * spatial_shape[1]
            src = src_flatten[:, start_idx:end_idx, :].transpose(1, 2).reshape(src_flatten.size(0), -1,
                                                                               spatial_shape[0], spatial_shape[1])
            mask = mask_flatten[:, start_idx:end_idx].reshape(mask_flatten.size(0), spatial_shape[0], spatial_shape[1])
            pos_embed = lvl_pos_embed_flatten[:, start_idx:end_idx, :].transpose(1, 2).reshape(
                lvl_pos_embed_flatten.size(0), -1, spatial_shape[0], spatial_shape[1])
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, is_recover=False):
        """
        Input:
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            re_spatial_shapes = [(src.size(-2), src.size(-1))]
        ...
        if is_recover:
            restored_srcs, restored_masks, restored_pos_embeds = self.inverse_transform(memory, mask_flatten,
                                                                                        lvl_pos_embed_flatten,
                                                                                        spatial_shapes)

        if self.two_stage_type =='standard':
            if self.two_stage_learn_wh:  # n
            ...
            # if is_recover:
            #     restored_srcs, restored_masks, restored_pos_embeds = self.inverse_transform(output_memory, mask_flatten,
            #                                                                                 lvl_pos_embed_flatten,
            #                                                                                 spatial_shapes)

            if self.two_stage_pat_embed > 0:  # n
            ...
            if self.two_stage_add_query_num > 0:  # n
            ...
            if self.embed_init_tgt:  # y
                tgt_ = self.tgt_embed.weight[:, None, :].repeat(1, bs, 1).transpose(0, 1) # nq, bs, d_model
            else:
                tgt_ = tgt_undetach.detach()

            if refpoint_embed is not None:  # y
        ...
        #########################################################     
        if self.two_stage_type == 'standard':
            if self.two_stage_keep_all_tokens:  # no
        ...
        #########################################################        
        # print(hs_enc, ref_enc)
        if is_recover:
            return hs, references, hs_enc, ref_enc, init_box_proposal, restored_srcs
        else:
            return hs, references, hs_enc, ref_enc, init_box_proposal, None
        # hs: (n_dec, bs, nq, d_model)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            dropflag = False
            if self.dec_layer_dropout_prob is not None:  # n
                prob = random.random()
