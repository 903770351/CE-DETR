...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes,
                 num_queries_one2one=900, num_queries_one2many=0,
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        if self.dn_number > 0 or targets is not None:
            input_query_label, input_query_bbox, attn_mask, dn_meta =\
                prepare_for_cdn(dn_args=(targets, self.dn_number, self.dn_label_noise_ratio, self.dn_box_noise_scale),
                                ...
                                num_queries_one2one=self.num_queries_one2one)
        else:
        ...
        if self.dn_number > 0 and dn_meta is not None:
            outputs_classes_one2one, outputs_coords_one2one = \
                dn_post_process(outputs_classes_one2one, outputs_coords_one2one,
                                dn_meta, self.aux_loss, self._set_aux_loss)
        out = {'pred_logits': outputs_classes_one2one[-1],
...
def build_dino(args):
    ...
    losses = ['labels', 'boxes', 'cardinality', 'vfl']
    if args.masks:
        losses += ["masks"]
